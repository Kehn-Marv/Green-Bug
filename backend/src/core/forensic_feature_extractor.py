...
class ForensicFeatureExtractor:
    ...
    def _extract_spectral_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['fft_low_energy_ratio'] = float(np.sum((magnitude_spectrum * low_mask) ** 2) / total_energy)
            features['fft_mid_energy_ratio'] = float(np.sum((magnitude_spectrum * mid_mask) ** 2) / total_energy)
            features['fft_high_energy_ratio'] = float(np.sum((magnitude_spectrum * high_mask) ** 2) / total_energy)
            ...
            features['dct_dc_ac_ratio'] = float(features['dct_dc_energy'] / (ac_energy + 1e-8))
    ...
    def _extract_color_statistics(self, img: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['rg_correlation'] = float(np.corrcoef(r, g)[0, 1])
            features['rb_correlation'] = float(np.corrcoef(r, b)[0, 1])
            features['gb_correlation'] = float(np.corrcoef(g, b)[0, 1])
            ...
            features['color_diversity'] = float(unique_colors / total_pixels)
    ...
    def _extract_texture_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['lbp_uniformity'] = float(np.sum(lbp_hist[:25]) / np.sum(lbp_hist))
    ...
    def _extract_cnn_features(self, image: Image.Image) -> Dict[str, float]:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                cnn_features = cnn_features.squeeze().numpy()
                
                # Statistical summary of CNN features
                features['cnn_mean'] = float(np.mean(cnn_features))
                features['cnn_std'] = float(np.std(cnn_features))
                features['cnn_max'] = float(np.max(cnn_features))
                features['cnn_min'] = float(np.min(cnn_features))
                features['cnn_l2_norm'] = float(np.linalg.norm(cnn_features))
                
                # Sparsity measure
                features['cnn_sparsity'] = float(np.sum(cnn_features == 0) / len(cnn_features))
                
                # Energy in different percentiles
                sorted_features = np.sort(np.abs(cnn_features))[::-1]
                total_energy = np.sum(sorted_features ** 2)
                
                for percentile in [10, 25, 50, 75, 90]:
                    idx = int(len(sorted_features) * percentile / 100)
                    energy = np.sum(sorted_features[:idx] ** 2)
                    features[f'cnn_energy_top_{percentile}pct'] = float(energy / total_energy)
    ...
    def _compute_spectrum_slope(self, radial_profile: np.ndarray) -> float:
        ...
        try:
            ...
            log_x = np.log(x[valid_idx])
            log_y = np.log(y[valid_idx])
    ...
    def _compute_symmetry(self, img: np.ndarray, axis: int) -> float:
        ...
        try:
            ...
            if axis == 0:
                ...
                correlation = np.corrcoef(top.flatten(), bottom.flatten())[0, 1]
            else:
                left = left[:, :min_dim]
                right = right[:, :min_dim]
    ...
    def _analyze_quantization_tables(self, image: Image.Image) -> Dict[str, float]:
        ...
        try:
            # This is a simplified analysis - in practice, you'd need to extract
            # actual quantization tables from JPEG headers
            import io
            ...
            for quality in [50, 75, 95]:
                ratio = features[f'compressed_size_q{quality}'] / original_size
    ...
    def _analyze_block_similarities(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if similarities:
                ...
                high_similarity_count = np.sum(np.array(similarities) > 0.8)
                features['high_similarity_block_ratio'] = float(high_similarity_count / len(similarities))
