...
class ForensicFeatureExtractor:
    ...
    def _extract_spectral_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['fft_low_freq_ratio'] = float(np.sum((magnitude_spectrum * low_freq_mask) ** 2) / total_energy)
            features['fft_mid_freq_ratio'] = float(np.sum((magnitude_spectrum * mid_freq_mask) ** 2) / total_energy)
            features['fft_high_freq_ratio'] = float(np.sum((magnitude_spectrum * high_freq_mask) ** 2) / total_energy)
            
            # DCT features
            dct = cv2.dct(gray.astype(np.float32))
            features['dct_energy'] = float(np.sum(dct ** 2))
            features['dct_ac_energy'] = float(np.sum(dct[1:, 1:] ** 2))
            features['dct_dc_ac_ratio'] = float(dct[0, 0] ** 2 / (features['dct_ac_energy'] + 1e-8))
    ...
    def _extract_color_statistics(self, img: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['rg_correlation'] = float(np.corrcoef(img[:, :, 0].flatten(), img[:, :, 1].flatten())[0, 1])
            features['rb_correlation'] = float(np.corrcoef(img[:, :, 0].flatten(), img[:, :, 2].flatten())[0, 1])
            features['gb_correlation'] = float(np.corrcoef(img[:, :, 1].flatten(), img[:, :, 2].flatten())[0, 1])
    ...
    def _extract_texture_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            features['lbp_uniformity'] = float(np.sum(lbp <= 24) / lbp.size)
