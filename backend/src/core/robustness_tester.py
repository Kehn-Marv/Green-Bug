...
# Pillow >= 9.1: Resampling enum may exist
try:
    Resampling: Optional[Type] = getattr(Image, 'Resampling', None)
except Exception:
    Resampling = None

# Define local constants with safe fallbacks for mypy
LANCZOS = getattr(Resampling, 'LANCZOS', getattr(Image, 'LANCZOS', None))
BICUBIC = getattr(Resampling, 'BICUBIC', getattr(Image, 'BICUBIC', None))
PERSPECTIVE = getattr(Image, 'PERSPECTIVE', None)
...
class RobustnessTester:
    ...
    def _coerce_score(self, score: Any) -> float:
        """Safely coerce a detector output to a native Python float.

        This helps avoid numpy scalar typing issues when mixing numpy.float64
        and built-in float in aggregates and builtin calls like min/max.
        """
        try:
            return float(score)
        except Exception:
            # If score is a dict or complex structure, try common fields
            try:
                if isinstance(score, dict):
                    # prefer 'deep_model_score' or 'score' keys if present
                    for k in ('deep_model_score', 'score', 'prob'):
                        if k in score:
                            return float(score[k])
            except Exception:
                pass
            self.logger.debug(f"Unable to coerce score to float: {score}")
            return 0.0
    ...
    def comprehensive_robustness_test(self, image: Image.Image,
        ...
        try:
            ...
            # stability_metrics is Dict[str, float]
            from typing import cast, Dict
            stability_metrics = cast(Dict[str, float], results["stability_metrics"])
            results["overall_robustness"] = self._compute_overall_robustness(stability_metrics)
    ...
    def _run_category_tests(self, category: str, tests: List[Dict],
        ...
        try:
            ...
            all_scores = [float(original_score)]
            ...
            for test_config in tests:
                try:
                    ...
                    # Normalize any returned scores to native floats
                    if isinstance(test_result, dict) and 'scores' in test_result:
                        try:
                            normalized = [self._coerce_score(s) for s in test_result['scores']]
                            test_result['scores'] = normalized
                            all_scores.extend(normalized)
                        except Exception:
                            # fallback: ignore problematic scores
                            test_result['scores'] = []

                    # Safely assign into typed tests dict to satisfy mypy
                    tests_dict: Dict[str, Any] = category_results.get("tests", {})
                    tests_dict[test_config['name']] = test_result
                    category_results["tests"] = tests_dict

                except Exception as e:
                    self.logger.error(f"Test {test_config['name']} failed: {e}")
                    tests_dict: Dict[str, Any] = category_results.get("tests", {})
                    tests_dict[test_config['name']] = {
                        "error": str(e),
                        "stability_score": 0.0
                    }
                    category_results["tests"] = tests_dict
            ...
            if len(all_scores) > 1:
                ...
                category_results["worst_case_score"] = float(min(all_scores))
                category_results["best_case_score"] = float(max(all_scores))
                category_results["score_variance"] = float(np.var(all_scores))
    ...
    def _test_resize_scaling(self, image: Image.Image, detector_func: Callable,
        ...
        try:
            ...
            for scale in scales:
                ...
                resample_filter = LANCZOS or Image.LANCZOS
                resized = image.resize(new_size, resample_filter)

                # Resize back to original size
                restored = resized.resize(original_size, resample_filter)
    ...
    def _test_crop_operations(self, image: Image.Image, detector_func: Callable,
        ...
        try:
            ...
            for ratio in ratios:
                ...
                # Resize back to original size
                resample_filter = LANCZOS or Image.LANCZOS
                restored = cropped.resize((w, h), resample_filter)
    ...
    def _test_rotation(self, image: Image.Image, detector_func: Callable,
        ...
        try:
            ...
            for angle in angles:
                ...
                resample_filter = LANCZOS or Image.LANCZOS
                restored = rotated.resize(image.size, resample_filter)
    ...
    def _test_perspective_transform(self, image: Image.Image, detector_func: Callable,
        ...
        try:
            ...
            for strength in strengths:
                ...
                perspective_const = PERSPECTIVE or getattr(Image, 'PERSPECTIVE', None)
                bicubic_const = BICUBIC or getattr(Image, 'BICUBIC', None)
                transformed = image.transform(
                    (w, h), perspective_const if perspective_const is not None else Image.PERSPECTIVE,
                    self._get_perspective_coeffs(src_points, dst_points),
                    bicubic_const if bicubic_const is not None else Image.BICUBIC
                )
    ...
    def _test_aspect_ratio_change(self, image: Image.Image, detector_func: Callable,
        ...
        try:
            ...
            for ratio in ratios:
                ...
                new_w = int(w * ratio)
                resample_filter = LANCZOS or Image.LANCZOS
                resized = image.resize((new_w, h), resample_filter)

                # Restore original aspect ratio
                restored = resized.resize((w, h), resample_filter)
    ...
    def _compute_stability_metrics(self, category_results: Dict[str, Any],
        ...
        try:
            ...
            if all_stabilities:
                metrics["mean_stability"] = float(np.mean(all_stabilities))
                metrics["min_stability"] = float(np.min(all_stabilities))
                metrics["max_stability"] = float(np.max(all_stabilities))
                metrics["stability_variance"] = float(np.var(all_stabilities))
        ...
        try:
            ...
            for category, results in category_results.items():
                ...
                for category, scores in score_by_category.items():
                    trend = float(np.mean(scores)) - float(original_score)
