...
class SafeguardMonitor:
    ...
    def check_training_safeguards(self, learning_db: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for check_func in self.safeguard_checks:
                try:
                    ...
                    if check_result["passed"]:
                        checks_passed = safeguard_results.setdefault("checks_passed", [])
                        if isinstance(checks_passed, list):
                            checks_passed.append(check_result)
                    else:
                        checks_failed = safeguard_results.setdefault("checks_failed", [])
                        if isinstance(checks_failed, list):
                            checks_failed.append(check_result)
                        safeguard_results["safe_to_train"] = False

                    if check_result.get("warnings"):
                        warnings = safeguard_results.setdefault("warnings", [])
            ...
            # Record safeguard check (ensure proper types before mutating)
            safeguards = learning_db.setdefault("safeguards", {})
            safeguards["last_check"] = datetime.now().isoformat()
            if not safeguard_results["safe_to_train"]:
                violations = safeguards.setdefault("violations", [])
                # cast to a typed list before append
                if isinstance(violations, list):
                    violations.append({
                        "timestamp": datetime.now().isoformat(),
                        "failed_checks": safeguard_results.get("checks_failed", [])
                    })
            # persist back in case learning_db was a mapping proxy
            learning_db["safeguards"] = safeguards
