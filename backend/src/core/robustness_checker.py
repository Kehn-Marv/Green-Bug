    ...
    def _compute_stability_metrics(self, test_results: Dict[str, Any], original_score: float) -> Dict[str, float]:
        ...
        try:
            ...
            for test_type, scores in test_types.items():
                if scores:
                    metrics[f"{test_type}_stability"] = np.mean(scores)
                    metrics[f"{test_type}_min_stability"] = np.min(scores)
                    metrics[f"{test_type}_max_stability"] = np.max(scores)
            ...
            if all_scores:
                metrics["mean_stability"] = np.mean(all_scores)
                metrics["min_stability"] = np.min(all_scores)
                metrics["max_stability"] = np.max(all_scores)
                metrics["stability_variance"] = np.var(all_scores)
    ...
    def _compute_overall_robustness(self, stability_metrics: Dict[str, float]) -> float:
        ...
        try:
            ...
            if total_weight > 0:
                ...
                return stability_metrics.get("mean_stability", 0.0)
