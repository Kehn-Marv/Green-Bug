...
class GradCAM:
    ...
    # hooks list for registered forward/backward hooks
    self.hooks: List[Any] = []
    ...
    def _register_hooks(self):
        ...
        try:
            ...
            def backward_hook(module, grad_input, grad_output):
                # grad_output may be a tuple; take first element
                try:
            ...
            # Use register_full_backward_hook if available, else fallback
            remove_fn = getattr(target_module, 'register_full_backward_hook', None)
            if remove_fn is not None:
                self.hooks.append(remove_fn(backward_hook))
            else:
                # Older PyTorch may have register_backward_hook
                try:
                    self.hooks.append(target_module.register_backward_hook(backward_hook))
                except Exception:
                    logger.warning("Backward hook registration not supported on this PyTorch version")
