...
# This file is a script-style API tester and not a pytest unit test.
# Prevent pytest from collecting it as a test module during automated runs.
pytest.skip("Skipping script-style API tester during pytest collection", allow_module_level=True)
...
def test_analyze_endpoint(base_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  Deep model score: {data.get('scores', {}).get('deep_model_score', 0):.3f}")
